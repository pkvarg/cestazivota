...
const ContactForm = ({ language, setShowContact }) => {
  ...
  const increaseBots = async () => {
    const apiUrl = 'https://hono-api.pictusweb.com/api/bots/cz/increase'
    //const apiUrl = 'http://localhost:3013/api/bots/cz/increase'
    try {
  ...
  const increaseEmails = async () => {
    const apiUrl = 'https://hono-api.pictusweb.com/api/emails/cz/increase'
    //const apiUrl = 'http://localhost:3013/api/emails/cz/increase'
    try {
      const { data } = await axios.put(apiUrl, {}, config)
    } catch (error) {
            ...
            <div>
              ...
              >
                <div className="flex flex-col justify-center">
                  <label className="form-label my-[2.5%] text-left">{content.contactEmail}</label>
                  <input
                    ...
                    required="required"
                  />
                  <label className="form-label my-[2.5%] text-left">{content.contactMessage}</label>
                  <textarea
                    className="form-control rounded-xl"
                    rows="5"
                    name="message"
